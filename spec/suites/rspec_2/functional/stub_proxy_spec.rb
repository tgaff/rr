  ...
  def self.include_tests_on_existing_methods(&block)
    ...
    context 'stubbing invocations of specific argument sets' do
      def self.argument_expectation_tests(&add_argument_expectation)
        ...
        it "is reset correctly" do
          object = build_object_with_method(:some_method) {|arg| 'existing value' }
          add_argument_expectation.call(stub.proxy(object), :some_method, 1).returns { 'new value' }
          RR.reset
          expect(object.some_method(1)).to eq 'existing value'
        end
